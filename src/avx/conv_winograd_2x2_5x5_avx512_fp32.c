...
#ifdef TENSOR_FORMAT_NHWC
void conv_winograd_2x2_5x5_avx512_fp32_nhwc_pre
#else
void conv_winograd_2x2_5x5_avx512_fp32_nchw_pre
#endif
...
#ifdef TENSOR_FORMAT_NHWC
void conv_winograd_2x2_5x5_avx512_fp32_nhwc_kernel
#else
void conv_winograd_2x2_5x5_avx512_fp32_nchw_kernel
#endif
    ...
    timt_h= 2;                     timt_w= 6;                     // Number of tiles per input macrotile: height and width
...
/*
                    WX[12] = _mm512_setzero_ps();
                    WX[13] = _mm512_setzero_ps();
                    WX[14] = _mm512_setzero_ps();
                    WX[15] = _mm512_setzero_ps();
*/
                    // Transpose Wk so that
                    // W0, W1, W2, W3 now contain the columns of the previous Wk
        ...
        for (ik = 0; ik < k; ik++)
            for (ih = 0; ih < tile_h; ih++)
                for (iw = 0; iw < tile_w; iw++) {
                    ...
                    // At = [      1,      1,      1,      1,      1,      0 ]
                    //      [      0,      1,     -1,      2,     -2,      1 ]
                    _WX[0] = MX[0] + MX[1] + MX[2] + MX[3] + MX[4];
                    _WX[1] = MX[1] - MX[2] + 2.0 * (MX[3] - MX[4]) + MX[5];
                    ...
                    // At = [      1,      1,      1,      1,      1,      0 ]
                    //      [      0,      1,     -1,      2,     -2,      1 ]
                    Z[0] = _WX[0][0] + _WX[0][1] + _WX[0][2] + _WX[0][3] + _WX[0][4];
                    Z[2] = _WX[1][0] + _WX[1][1] + _WX[1][2] + _WX[1][3] + _WX[1][4];

                    Z[1] = _WX[0][1] - _WX[0][2] + 2.0 * _WX[0][3] - 2.0 * _WX[0][4] + _WX[0][5];
                    Z[3] = _WX[1][1] - _WX[1][2] + 2.0 * _WX[1][3] - 2.0 * _WX[1][4] + _WX[1][5];
...
#ifdef TENSOR_FORMAT_NHWC
void conv_winograd_2x2_5x5_avx512_fp32_nhwc
#else
void conv_winograd_2x2_5x5_avx512_fp32_nchw
#endif
        (int m, int r, int n, int k, int c,
         int hi, int wi, int kh, int kw,
         int vpadding, int hpadding,
...
#ifdef TENSOR_FORMAT_NHWC
    conv_winograd_2x2_5x5_avx512_fp32_nhwc_pre
#else
...
#ifdef TENSOR_FORMAT_NHWC
    conv_winograd_2x2_5x5_avx512_fp32_nhwc_kernel
#else
    conv_winograd_2x2_5x5_avx512_fp32_nchw_kernel
#endif
