 ...
 if ( (n==0)||(k==0)||(c==0)||
  ...
  for (ik = 0; ik < k; ik++)
    for (ic = 0; ic < c; ic++) {
      ...
      //   [      0,      0,      1 ]                            [W50,  W51,  W52]
      W0 =  (float)(1.0/4.0)*F0;
      W1 = (float)(-1.0/6.0)*F0 -  (float)(1.0/6.0)*F1 - (float)(1.0/6.0)*F2;
      W2 = (float)(-1.0/6.0)*F0 +  (float)(1.0/6.0)*F1 - (float)(1.0/6.0)*F2;
      W3 = (float)(1.0/24.0)*F0 + (float)(1.0/12.0)*F1 + (float)(1.0/6.0)*F2;
      W4 = (float)(1.0/24.0)*F0 - (float)(1.0/12.0)*F1 + (float)(1.0/6.0)*F2;
      W5 =                                                                F2;
      ...
      U0  =  (float)(1.0/4.0)*W0;
      U1  = (float)(-1.0/6.0)*W0    -  (float)(1.0/6.0)*W1    - (float)(1.0/6.0)*W2;
      U2  = (float)(-1.0/6.0)*W0    +  (float)(1.0/6.0)*W1    - (float)(1.0/6.0)*W2;
      U3  = (float)(1.0/24.0)*W0    + (float)(1.0/12.0)*W1    + (float)(1.0/6.0)*W2;
      U4  = (float)(1.0/24.0)*W0    - (float)(1.0/12.0)*W1    + (float)(1.0/6.0)*W2;
      U5  =                                                                      W2;   

      U04 =           1.0/4.0*W4[0];
      U14 =          -1.0/6.0*W4[0] -           1.0/6.0*W4[1] -          1.0/6.0*W4[2];
      U24 =          -1.0/6.0*W4[0] +           1.0/6.0*W4[1] -          1.0/6.0*W4[2];
      U34 =          1.0/24.0*W4[0] +          1.0/12.0*W4[1] +          1.0/6.0*W4[2];
      U44 =          1.0/24.0*W4[0] -          1.0/12.0*W4[1] +          1.0/6.0*W4[2];
      U54 =                                                                      W4[2];

      U05 =           1.0/4.0*W5[0];
      U15 =          -1.0/6.0*W5[0] -           1.0/6.0*W5[1] -          1.0/6.0*W5[2];
      U25 =          -1.0/6.0*W5[0] +           1.0/6.0*W5[1] -          1.0/6.0*W5[2];
      U35 =          1.0/24.0*W5[0] +          1.0/12.0*W5[1] +          1.0/6.0*W5[2];
      U45 =          1.0/24.0*W5[0] -          1.0/12.0*W5[1] +          1.0/6.0*W5[2];
      U55 =                                                                      W5[2];
    ...
    for (ic = 0; ic < c; ic++)
      for (ih = 0; ih < tile_h; ih++) {
        ...
        for (iw = 0; iw < tile_w; iw++) {
          ...
          W0     = (float)(4.0)*d0                    - (float)(5.0)*d2                   + d4;
          W1     =                   (float)(-4.0)*d1 - (float)(4.0)*d2 +              d3 + d4;
          W2     =                    (float)(4.0)*d1 - (float)(4.0)*d2 -              d3 + d4;
          W3     =                   (float)(-2.0)*d1 -              d2 + (float)(2.0)*d3 + d4;
          W4     =                    (float)(2.0)*d1 -              d2 - (float)(2.0)*d3 + d4;
          W5     =                    (float)(4.0)*d1                   - (float)(5.0)*d3         + d5;

          W4_[0] =   4.0*drow(0,4) -                      5.0*drow(2,4)                   + drow(4,4);
          W4_[1] =                 -    4.0*drow(1,4) -   4.0*drow(2,4) +       drow(3,4) + drow(4,4);
          W4_[2] =                      4.0*drow(1,4) -   4.0*drow(2,4) -       drow(3,4) + drow(4,4);
          W4_[3] =                 -    2.0*drow(1,4) -       drow(2,4) +   2.0*drow(3,4) + drow(4,4);
          W44    =                      2.0*drow(1,4) -       drow(2,4) -   2.0*drow(3,4) + drow(4,4);
          W45    =                      4.0*drow(1,4)                   -   5.0*drow(3,4)             + drow(5,4);

          W5_[0] =   4.0*drow(0,5) -                      5.0*drow(2,5)                   + drow(4,5);
          W5_[1] =                 -    4.0*drow(1,5) -   4.0*drow(2,5) +       drow(3,5) + drow(4,5);
          W5_[2] =                      4.0*drow(1,5) -   4.0*drow(2,5) -       drow(3,5) + drow(4,5);
          W5_[3] =                 -    2.0*drow(1,5) -       drow(2,5) +   2.0*drow(3,5) + drow(4,5);
          W54    =                      2.0*drow(1,5) -       drow(2,5) -   2.0*drow(3,5) + drow(4,5);
          ...
          U0     = (float)(4.0)*W0                           - (float)(5.0)*W2                           + W4_;
          U1     =                     - (float)(4.0)*W1     - (float)(4.0)*W2     +              W3     + W4_;
          U2     =                       (float)(4.0)*W1     - (float)(4.0)*W2     -              W3     + W4_;
          U3     =                     - (float)(2.0)*W1     -              W2     + (float)(2.0)*W3     + W4_;
          U4     =                       (float)(2.0)*W1     -              W2     - (float)(2.0)*W3     + W4_;
          U5     =                       (float)(4.0)*W1                           - (float)(5.0)*W3           + W5_;

          U04    =          4.0*W4[0]                        -           5.0*W4[2]                       + W44;
          U14    =                               - 4.0*W4[1] -           4.0*W4[2] +               W4[3] + W44;
          U24    =                                 4.0*W4[1] -           4.0*W4[2] -               W4[3] + W44;
          U34    =                               - 2.0*W4[1] -               W4[2] +           2.0*W4[3] + W44;
          U44    =                                 2.0*W4[1] -               W4[2] -           2.0*W4[3] + W44;
          U54    =                                 4.0*W4[1]                       -           5.0*W4[3]       + W54;

          U05    =          4.0*W5[0]                        -           5.0*W5[2]                       + W45;
          U15    =                               - 4.0*W5[1] -           4.0*W5[2] +               W5[3] + W45;
          U25    =                                 4.0*W5[1] -           4.0*W5[2] -               W5[3] + W45;
          U35    =                               - 2.0*W5[1] -               W5[2] +           2.0*W5[3] + W45;
          U45    =                                 2.0*W5[1] -               W5[2] -           2.0*W5[3] + W45;
    ...
    for (ik = 0; ik < k; ik++)
      for (ih = 0; ih < tile_h; ih++)
        for (iw = 0; iw < tile_w; iw++) {
          ...
          W0     =  M0          + M1          + M2          +     M3          +     M4;
          W1     =                M1          - M2          + 2.0*M3          - 2.0*M4;
          W2     =                M1          + M2          + 4.0*M3          + 4.0*M4;
          W3     =                M1          - M2          + 8.0*M3          - 8.0*M4          + M5;

          W4_[0] =  Mprow(0,4) + Mprow(1,4) + Mprow(2,4) +     Mprow(3,4) +     Mprow(4,4);
          W4_[1] =               Mprow(1,4) - Mprow(2,4) + 2.0*Mprow(3,4) - 2.0*Mprow(4,4);
          W4_[2] =               Mprow(1,4) + Mprow(2,4) + 4.0*Mprow(3,4) + 4.0*Mprow(4,4);
          W4_[3] =               Mprow(1,4) - Mprow(2,4) + 8.0*Mprow(3,4) - 8.0*Mprow(4,4) + Mprow(5,4);
 
          W5_[0] =  Mprow(0,5) + Mprow(1,5) + Mprow(2,5) +     Mprow(3,5) +     Mprow(4,5);
          W5_[1] =               Mprow(1,5) - Mprow(2,5) + 2.0*Mprow(3,5) - 2.0*Mprow(4,5);
          W5_[2] =               Mprow(1,5) + Mprow(2,5) + 4.0*Mprow(3,5) + 4.0*Mprow(4,5);
          W5_[3] =               Mprow(1,5) - Mprow(2,5) + 8.0*Mprow(3,5) - 8.0*Mprow(4,5) + Mprow(5,5);
          ...
          //                                                           [ W05,  W15,  W52,  W53 ]
          Z0     =     W0    +     W1    +     W2    +     W3    +     W4_;
          Z1     =                 W1    -     W2    + 2.0*W3    - 2.0*W4_;
          Z2     =                 W1    +     W2    + 4.0*W3    + 4.0*W4_;
          Z3     =                 W1    -     W2    + 8.0*W3    - 8.0*W4_   +    W5_;
